# bitmex_swagger.FundingApi

All URIs are relative to *https://localhost/api/v1*

Method | HTTP request | Description
------------- | ------------- | -------------
[**funding_get**](FundingApi.md#funding_get) | **GET** /funding | Get funding history.


# **funding_get**
> list[Funding] funding_get(symbol=symbol, filter=filter, columns=columns, count=count, start=start, reverse=reverse, start_time=start_time, end_time=end_time)
...
api_instance = bitmex_swagger.FundingApi()
...
try:
    # Get funding history.
    api_response = api_instance.funding_get(symbol=symbol, filter=filter, columns=columns, count=count, start=start, reverse=reverse, start_time=start_time, end_time=end_time)
    pprint(api_response)
except ApiException as e:
    print("Exception when calling FundingApi->funding_get: %s\n" % e)
```
